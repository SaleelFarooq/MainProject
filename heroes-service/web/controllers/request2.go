...
func (app *Application) RequestHandler2(w http.ResponseWriter, r *http.Request) {
	...
	if r.FormValue("requested") == "true" {
		...
		a = queryResponse[0].Value.Key
		b = queryResponse[0].Value.IMEINo
		c = queryResponse[0].Value.Specifications
		d = queryResponse[0].Value.ProducerName
		sh = queryResponse[0].Value.ShelfLife
		f = queryResponse[0].Value.ManufacturingSite
		g = queryResponse[0].Value.FinalAssemblyDate
	}
